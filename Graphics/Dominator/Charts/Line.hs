...
linePlotDay :: Double -- ^ width in pixels
            ...
            -> [(Day, Double)] -- ^ points
            -> Canvas2D
linePlotDay width height xScale yScale yLabels marker points =
  ...
  in
   linePlot width height xScale xLabels yScale yLabels marker points'
...
linePlot width' height' xScale xLabels yScale yLabels marker points =
  let paddingTop = 20
      ...
      drawXAxis height toXPos (x, label) =
        ...
        , Draw (Stroke [ MoveTo (toXPos x) (height+paddingTop+10)
...
--       , Draw (FillText "A Scatter Plot" 1 40 Nothing)
        ...
        ] ++ (map (drawMarker toXPos toYPos marker) points) ++ (connectTheDots toXPos toYPos points))
